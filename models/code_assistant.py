...
class CodeAssistant:
    ...
    def process_task(self, task_type: TaskType, prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Format prompt
            formatted_prompt = PromptTemplate(config.prompt_template).format(prompt=prompt)

            llm = self.llms.get(config.model_name)
            response_text = None
            ...
            try:
                ...
                logger.warning(f"[OFFLINE MODE] Model not available ({e}) — using local fallback.")
                response_text = None

            # ---------------------------------------
            # FALLBACK for offline / failed API
            # ---------------------------------------
            if not response_text or not response_text.strip():
                logger.info("Running offline fallback logic.")
                ...
                if "c++" in lower_prompt:
                    lang = "cpp"
                elif "java" in lower_prompt:
                    lang = "java"

                # 🔹 Local fallback templates
                if "fibonacci" in lower_prompt:
                    if lang == "python":
                        fallback_code = """# Python: Generate Fibonacci sequence
        def fibonacci(n):
            a, b = 0, 1
            for _ in range(n):
                print(a)
        ...
        """
                        ...
                        fallback_code = """// C++: Generate Fibonacci sequence
        #include <iostream>
        using namespace std;
        ...
        """
                        ...
                        fallback_code = """// Java: Generate Fibonacci sequence
        ...
        """
                    ...
                    fallback_code = "# Offline fallback: No AI model connected, returning template code."

                return self._handle_code_generation(prompt, fallback_code)

            # ---------------------------------------
            # If model succeeded, continue normally
            # ---------------------------------------
            response_text = response_text.strip()

            if task_type == TaskType.CODE_GENERATION:
                return self._handle_code_generation(prompt, response_text)
            elif task_type == TaskType.CODE_REVIEW:
                return self._handle_code_review(response_text)
            elif task_type == TaskType.DOCUMENTATION:
                return self._handle_documentation(response_text)
            elif task_type == TaskType.TESTING:
                return self._handle_testing(prompt, response_text)
